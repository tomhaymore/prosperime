...
from social.models import Comment, Conversation
...
class FeedBase():
	...
	def get_univ_feed(self,user,**opts):
		...
		# threads = Conversation.objects.filter(Q(owner__in=connections) | Q(owner__in=educations)).filter(updated__gte=stop)
		...
		unordered_feed.extend([{'type':'goalposition','id':p.position.id,'user_id':p.owner_id,'user_name':p.owner.profile.full_name(),'title':p.position.title,'body':None,'target_user_name':None,'target_user_id':None,'target_type':None,'target_name':None,'target_id':None,'date':p.updated,'stub':{'user_id':p.owner_id,'user_name':p.owner.profile.full_name(),'user_pic':p.owner.profile.default_profile_pic(),'connected':p.owner_id in connections,'saved_paths':len(p.owner.savedPath.all()),'saved_positions':len(p.owner.goal_position.all()),'path':None,'career':None,'position':p.position.title, 'id':p.position.id, 'comment':None}} for p in positions])
		## This was in merge and can't remember which is which
		# unordered_feed.extend([{'type':'goalposition','id':p.id,'user_id':p.owner_id,'user_name':p.owner.profile.full_name(),'title':p.position.title,'body':None,'target_user_name':None,'target_user_id':None,'target_type':None,'target_name':None,'target_id':None,'date':p.updated,'stub':{'user_id':p.owner_id,'user_name':p.owner.profile.full_name(),'user_pic':p.owner.profile.default_profile_pic(),'connected':p.owner_id in connections,'saved_paths':len(p.owner.savedPath.all()),'saved_positions':len(p.owner.goal_position.all()),'path':None,'career':None,'position':{'title':p.position.title,'id':p.position.id},'comment':None}} for p in positions])
		...
		unordered_feed.extend([
			...
			'comment':{
				'body':c.body,
				'commenter_id':c.owner_id,
				'commenter_name':c.owner.profile.full_name(),
