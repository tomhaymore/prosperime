...
def _get_careers_brief_in_network(user,**filters):
	...
	## Clay added one _ after person to make validate ##
	careers = Career.objects.filter(positions__person__id__in=users).annotate(num_people=Count('positions__person__pk'),num_pos=Count('positions__pk'),num_cos=Count('positions__entity__pk')).distinct()
